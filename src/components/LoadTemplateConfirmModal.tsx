...
type Props = {
  open: boolean;
  /** 置き換え対象の現在のスライド枚数（1 以上） */
  currentSlideCount: number;
  onClose: () => void;
  onConfirm: () => void;
};

export function LoadTemplateConfirmModal({ open, currentSlideCount, onClose, onConfirm }: Props) {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    ...
    >
      <div
        role="alertdialog"
        aria-modal="true"
        aria-labelledby="load-template-modal-title"
        aria-describedby="load-template-modal-desc"
      ...
      >
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-[#5b5c64]/15 px-5 py-4">
          <div className="min-w-0">
            <h2 id="load-template-modal-title" className="text-lg font-bold text-[#1C1C1E]">
              テンプレートを読み込みますか？
            </h2>
            <p id="load-template-modal-desc" className="mt-2 text-sm leading-relaxed text-[#5b5c64]">
              現在のスライド{' '}
              <span className="font-semibold text-[#1C1C1E]">{currentSlideCount} 枚</span>
              をすべて、標準テンプレートのスライドに置き換えます。編集中の内容は失われ、この操作は元に戻せません（あらかじめ
              JSON で書き出していない場合は復元できません）。
            </p>
          </div>
          <button
            type="button"
